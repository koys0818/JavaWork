...
-- 일단 부모테이블의 name을 UNIQUE 로 바꾼뒤 위의 쿼리를 다시 실행해보세요

ALTER TABLE t_emp2 ADD CONSTRAINT emp4_name_uk UNIQUE(name);
 depton varcharon DELETE NULL 
 
SELECT * FROM T_NOVALIDATE;
SELECT * FROM T_VALIDATE ;
SELECT owner, CONSTRAINT_name , CONSTRAINT_type,status
FROM USER_consraint
WHERE TABLE_name  sys_c007-
