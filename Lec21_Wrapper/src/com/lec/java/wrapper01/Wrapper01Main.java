...
public class Wrapper01Main {
	...
	public static void main(String[] args) {
		...
		if(num1 == num2) {
			...
			System.out.println("== 다르다!");		//다름
		}
		
		
		if(num1.equals(num2)) {
			System.out.println("equals() 같다");		//같음
		}else {
		...
		//vaiueOf()는 생성한 object를 cache 해둔다
		// 동일 literal로 valueOf()를 생성하면 기존의 생성된 object 리턴
		//왜?? ... 메모리 절약  --> == 같다
		...
		if(num1 == num3) {
			...
			System.out.println("생성자 : 다른 참조");	//다름
		}
		...
		// 6. 산술 연산 가능
		num6 *= 2;		// Wrapper의 연산결과는  새로운 Wrapper 생성
		System.out.println(num6);
		if(num5 == num6) {
			System.out.println("같은 참조");
			
		}else {
			System.out.println("다른 참조");	//다르다 wrapper는 immutable하다			
		}
